<?php
    ...
    $id =  $_GET['id'];
    $sql="select * from prize where id_prize=$id";
    // echo $sql;
    $result=mysqli_query($conn,$sql);
    $bn= mysqli_fetch_array($result);

    // ktra xem ng dùng đã gửi dl lên chưa 
    if( isset($_GET['content_prize'])  ){
        $content_prize= $_GET['content_prize'];
        
        //b2: truy vấn 
        $sql= "update prize set content_prize='$content_prize' where id_prize=$id";
        // echo $sql;
        $result=mysqli_query($conn,$sql);
        if($result) {
           header("Location:prize.php");
  ...
  <?php
    ...
    <input type="hidden" name="id" value="<?php  echo $bn['id'] ?>">   
        ...
        <div class="form-group">
            ...
            <input type="text" name="content_prize" id="content_prize" class="form-control" placeholder="" aria-describedby="helpId"  value=" <?php  echo $bn['content_prize'] ?>">
        </div>
